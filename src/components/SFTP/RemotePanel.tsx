...
export default function RemotePanel({ sessionId, onDownload, onDropUpload, draggedJob }: RemotePanelProps) {
  ...
  const handleRemoteCompress = async (file: SFTPFile, archiveType: 'zip' | 'tar.gz') => {
    ...
    const result = await window.sshApi.sftpRemoteCompress({
      sessionId,
      remotePath: `${currentPath}/${file.filename}`,
      archiveName,
      archiveType,
    })
  ...
  const handleRemoteExtract = async (file: SFTPFile) => {
    setContextMenu(null)
    setArchiveModal({ open: true, title: `Mengekstrak ${file.filename}` })

    const result = await window.sshApi.sftpRemoteExtract({
      sessionId,
      archivePath: `${currentPath}/${file.filename}`,
    })
  ...
  const ARCHIVE_EXTENSIONS = ['.zip', '.tar.gz', '.tgz', '.tar.bz2', '.tar.xz', '.tar', '.gz', '.bz2']
      ...
      {contextMenu && (
        ...
        >
          ...
          {isArchiveFile(contextMenu.file.filename) && (
            <button
              onClick={() => handleRemoteExtract(contextMenu.file)}
